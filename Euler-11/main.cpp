...
string print2DIntArray(int array[20][20])
{
    string out = "";
    for (int x = 0; x < 20; x++) {
        for (int y = 0; y < 20; y++) {
...
bool areAdjacent(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
{
        ...
        if (y1 + 1 == y2 && x1 + 1 == x2) {
            if ((y2 + 1 == y3) && (y3 + 1 == y4) && (x2 + 1 == x3) && (x3 + 1 == x4)) {
                return true;
            } else {
               return false;
            }
...
int main() {
   ...
   //  cout << endl << y << "\t|" << "\t";
    ...
    int y1 = 0, y2 = 0, y3 = 0, y4 = 0, highest = 0, now = 0;
           ...
           for (int x3 = x2 + 1; x3 < 20; x3++) {
               ...
               for (int x4 = x3 + 1; x4 < 20; x4++) {
                   if (areAdjacent(x1, y1, x2, y2, x3, y3, x4, y4)) {
                        ...
                        if (now > highest) {
                           cout << endl << x1 << "\t" << y1 << endl << x2 << "\t" << y2 << endl << x3 << "\t" << y3 << endl << x4 << "\t" << y4 << endl; 
                            highest = now;
                        }                  
                   if (x4 == 19) {
                       x4 = 0;
